...
TEST_CASE("Operator <<") {
    ...
    streamB << b;
    CHECK(streamB.str() == "10[kg]");    
}

TEST_CASE("Operator >>"){
    NumberWithUnits a{13,"ton"};
...
TEST_CASE("Operator - unari"){
    NumberWithUnits possitive{12,"hour"};
    CHECK((-possitive).getNumber()==-12);
    NumberWithUnits negative{-12,"hour"};
    CHECK((-negative).getNumber()==12);
}
